...
use axum::{Router, extract::{Path, Query, State}, http::StatusCode, routing::{get, post}};
...
async fn short(
    Query(params): Query<GetOrPostUrl>,
    State(db): State<Db>
...
async fn get_url(State(db): State<Db>, Path(url): Path<String>) -> Result<String, StatusCode> {
    let true_url_result = db.get_link(url).await;
